#!/usr/bin/env python
#############################################################################
# generate a checkpoints.json for Electrum, runs locally with a coin daemon #
# usage:  python getcheckpoints.py <rpcusername> <rpcpassword> <rpcport>    #
#############################################################################
import json
import sys
...
# For Electrum checkpoints.json generation
...
# Electrum checkpoints are between blocks 2015, 2015 + 2016, 2015 + 2016*2, ...
i = 2015
INTERVAL = 2016

checkpoints = []
block_count = int(rpc('getblockcount', [])['result'])
print(('Current Blocks: {}'.format(block_count)))
while True:
